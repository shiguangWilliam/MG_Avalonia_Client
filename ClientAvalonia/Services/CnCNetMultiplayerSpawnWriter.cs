...
public static class CnCNetMultiplayerSpawnWriter
{
        ...
        int randomSeed)
    {
        ...
        Logger.Log("CnCNetMultiplayerSpawnWriter: writing spawn.ini");

        var portsByName = new Dictionary<string, int>(StringComparer.OrdinalIgnoreCase);
        foreach (KeyValuePair<string, int> pair in startInfo.PlayerPorts)
        {
            if (CnCNetPortValidator.IsValidPlayerPort(pair.Value))
        ...
        if (portsByName.Count == 0 && roomPlayers != null)
        {
            ...
            {
                if (CnCNetPortValidator.IsValidPlayerPort(player.Port))
        ...
        int localPort = startInfo.LocalPlayerPort;
        ...
        for (int humanIndex = 0; humanIndex < humans.Count; humanIndex++)
        {
            ...
            if (portsByName.TryGetValue(human.Name, out int remotePort) && remotePort > 0)
