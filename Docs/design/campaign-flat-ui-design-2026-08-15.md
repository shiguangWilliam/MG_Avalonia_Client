...
11. **Classic 原版皮肤（2026-08-15 增补，人工核验）**：Classic 状态下启动器**使用原版皮肤**——战役窗口走通用 INI 模板（贴图驱动），不再路由到扁平化重设计的 `DxCampaign*` 模板（该模板集仅 Tactical 使用）；`EnsureCampaignControlSizes` 的合成尺寸仅在 Tactical 生效，避免破坏原版贴图宽高比；悬浮面板铬走 `DxOverlayChrome*` token 随主题切换。
...
**2026-08-15 修订（人工核验后）**：初版「星域+扫描环」被判定与创世之刻无关联。已重写为「创世瞬间」叙事构图，并改用**固定品牌色板（炭黑/金/熔红），不随用户 accent 变化**——创世之刻的视觉识别不能被个性化配色稀释：

| 元素 | 设计 | 实现 |
|---|---|---|
| 创世地平线 | 下三分之一一道金色圆弧——**新世界正从地平线上升起**（Mod 名的图像化），弧下熔红底光呼吸 | 巨圆 `EllipseGeometry` 顶点切于地平线 + RadialGradient 呼吸光晕 |
| 熔融核心 | 弧顶上方悬浮的金线框十二面体，**核心处脉动的熔红「心脏」**（新世界的种子在灼热中成形） | 3D 投影棱线（前金后暗）+ 三层径向脉动 |
| 对旋轨道环 | 核心外金/红双环反向旋转（浑天仪式能量约束场），环上各有行进光点 | 旋转扁椭圆 + 参数化行进火花 |
| 上升余烬 | 地平线向核心升起的火星粒子，接近核心时转金 | 正弦轨迹 + 相位随机 |
| 点火光丝 | 地平线到核心的单根金色能量丝，缓慢呼吸——「点燃」的瞬间 | 渐变竖丝 + 呼吸透明度 |

原先的星域视差、扫描环、引导光束已全部移除（与创世叙事无关）。

**配套修复**：Tactical 下主菜单根节点跳过不透明 `CreateWindowChrome`（否则创世背景被完全遮盖，这是此前「主界面与创世之刻无关联」的直接原因之一）；悬浮面板铬（Options/战役/建房）从硬编码暖色改为 `DxOverlayChrome*` 主题 token，随主题切换。
